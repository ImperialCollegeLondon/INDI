...
def get_data():
    ...
    info = {
        ...
        "n_slices": len(table["slice_integer"].unique()),
    }
...
def test_quick_tensor_fit(get_data):
    ...
    for i in range(info["n_slices"]):
        tensor_calculated.append(quick_tensor_fit(i, table, info))
    tensor_calculated = np.array(tensor_calculated)
...
def test_tensor_fitting(method, quick_mode, get_data, dummy_settings):
    ...
    context = {
        "data": data,
        "info": info,
        "mask_3c": convert_array_to_dict_of_arrays(mask_3c, np.arange(info["n_slices"])),
        "slices": np.arange(info["n_slices"]),
